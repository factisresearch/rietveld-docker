...
class PublishForm(forms.Form):
  ...
  reviewers = forms.CharField(required=False,
                              max_length=1000,
                              widget=AccountInput(attrs={'size': 60}))
# SW 2010-12-13: email sending with CC does not work
  cc = forms.CharField(required=False,
                       max_length=1000,
                       label = 'CC',
                       widget=AccountInput(attrs={'size': 60}))
  ...
  for other in patchsets:
    ...
    if other.data or other.parsed_patches:
      ...
      if other.parsed_patches is None:
        ...
        except UnicodeDecodeError, e:  # Fallback to str - unicode comparison.
          logging.warn("failed to decode data of patchset %s using encoding utf-8: %s" \
                       % (other.key().id(), e))
...
# SW: 2011-04-04: don't change subject when sending email
#  if request.user == issue.owner:
...
# sw, 2014-05-26, added
def _get_patch_id(descr):
    for l in descr.split('\n'):
        if l.startswith('(Hash:'):
            i = l.rfind('-')
            j = l.rfind(')')
            if i >= 0 and j >= 0 and i + 1 < j:
                h = l[i + 1 : j]
                return h
    return None
...
def _make_message(request, issue, message, comments=None, send_mail=False,
  ...
  if send_mail:
    ...
    # SW 2010-12-13: send_args['cc'] does not work, changed to send_args['to']
    if cc:
      send_args['to'] = send_args['to'] + [_encode_safely(address) for address in cc]
